...
| `fx.content.faq_absent` | extract | low | low | medium | medium | site | No `FAQPage` JSON-LD and no heading matching FAQ/questions patterns on any sampled page. Category-gated. Absence-scoped by the orchestrator (rubric rule 6) on the pricing, product and about pages the category expects: none read → `not_evaluated`, some read → a low-confidence sample claim. |
...
| `ef.entity.nap_missing_plain_text` | entity | medium | high | high | low | site | The organisation name and its locator facts are not visible text on the home, contact or about page. Required: `local_business` name + postal address + phone; `portfolio_personal` name + a contact (email or phone); `unknown` name + any contact (address, phone or email), because the audit does not demand a postal address of a site it cannot classify; every other category name + (postal address or phone). Text inside JSON-LD or images does not count. The name component is skipped when the brand name could only be derived from the host. Category overrides apply. Absence-scoped by the orchestrator (rubric rule 6) on the contact and about pages: neither read → `not_evaluated` (`role_page_not_sampled`), one read → a low-confidence sample claim naming the page not reached. |
...
| `ef.corroboration.press_page_missing` | corroboration | low | medium | medium | medium | site | No sampled page has the `blog` role, no internal link on a usable page has text matching press/news/newsroom/media/blog/updates/announcements/stories/insights/journal (or a first path segment among those), and no RSS/Atom feed is advertised. `mailto:` links never count. Category overrides apply. `not_evaluated` with reason `navigation_not_readable` (orchestrator, rubric rule 6) when the sample was too thin to have shown the navigation: three or fewer internal links on home, or at most one role page reached. |
...
| `en.cta.missing` | engagement | medium | medium | medium | low | per-page | On home and the category's key pages: no `<a>`/`<button>` whose text matches the category CTA vocabulary within the first 40% of the content markup (measured from the first visible content, not from `<body>`) or, for a link, inside `<header>`/`<nav>` at any markup position, and no non-search `<form>` there. `mailto:`/`tel:` links count where the vocabulary has email, contact or call. |
...
| `en.links.broken_sampled` | engagement | medium | medium | high | low | site | Of ≤ 15 sampled internal links (nav first, then header, body, footer; already-sampled pages reuse their status), ≥ 1 returns 404/410 or a 5xx. 403/429 are never broken (see the cluster row). A DNS/TLS/timeout/connection-refused transport error is retried once; if it persists with no real 404/410/5xx among the sample, the check is `inconclusive` (reason `fetcher_error`), never a fail -- the audit's own connection may be at fault, not the site. Evidence lists each. |
